...
# From Dataset 1 
# Do bat behaviours (bat_landing_to_food) changes as seasonal changes?
...
summer_data = Dataset_1[Dataset_1["season"].str.lower() == "summer"]
winter_data = Dataset_1[Dataset_1["season"].str.lower() == "winter"]
...
def run_models(df, season_type):
    ...
    # slr_results = []
    # print("\nSimple Linear Regression (SLR) Results (" + season_type + "):")
    ...
    #     rmse_slr = np.sqrt(mae_slr)
    #     r2_slr = r2_score(y_test, y_prediction_slr)
    #     coefficient_slr = model.coef_[0]
    #     intercept_slr = model.intercept_

    #     slr_results.append([feature, coefficient_slr, intercept_slr, mae_slr, mse_slr, rmse_slr, r2_slr])

    #     print(f"\nPredictor: {feature}")
    #     print(f"Coefficient: {coefficient_slr:.4f}")
    #     print(f"Intercept: {intercept_slr:.4f}")
    #     print(f"R-squared: {r2_slr:.4f}")
    #     print(f"MAE: {mae_slr:.4f}, RMSE: {rmse_slr:.4f}")

    #     slr_df = pd.DataFrame(slr_results, columns=["Feature", "Coefficient", "Intercept", "MAE", "MSE", "RMSE", "R2"])
